    ...
    <main className="p-4 md:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" onClick={() => { fetchPerusahaan(); if (tab === 'dokumen') fetchDocuments(); }} className="rounded-xl">
              ...
              <div className="flex items-center gap-4 min-w-0">
                <div className="h-14 w-14 rounded-2xl bg-white/15 ring-1 ring-white/25 flex items-center justify-center overflow-hidden shrink-0">
                  {loading ? (
                    ...
                    <img src={`${perusahaan.logoUrl}?t=${Date.now()}`} alt="Logo" className="h-12 w-12 object-contain" />
                  ) : (
                ...
                <div className="min-w-0">
                  {loading ? (
                    ...
                    <>
                      <h1 className="text-xl sm:text-2xl font-extrabold truncate">{perusahaan?.name}</h1>
                      <p className="text-xs sm:text-sm text-emerald-100 font-semibold mt-1">ID: {perusahaan?.id}</p>
                    </>
                  )}
                </div>
              </div>
          ...
          <div className="p-4 sm:p-6 bg-white">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                ...
                <div className="text-sm font-semibold text-gray-900 mt-1">{loading ? '-' : (perusahaan?.email || '-')}</div>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                <div className="text-[10px] font-black tracking-wider uppercase text-gray-400 flex items-center gap-1">
                ...
                <div className="text-sm font-semibold text-gray-900 mt-1">{loading ? '-' : (perusahaan?.phone || '-')}</div>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 sm:col-span-3">
                ...
                <div className="text-sm font-semibold text-gray-900 mt-1">{loading ? '-' : (perusahaan?.address || '-')}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <TabsContent value="harta">
            ...
            <Dialog open={assetModalOpen} onOpenChange={setAssetModalOpen}>
              <DialogContent className="bg-white w-[95vw] sm:max-w-lg rounded-2xl p-0 overflow-hidden [&>button.absolute]:hidden">
                ...
                <ModalContentWrapper className="space-y-4">
                  ...
                  <div className="text-xs text-gray-500">
                    Metode: Garis lurus sesuai kelompok penyusutan pajak.
                  </div>
                </ModalContentWrapper>
                <ModalFooter className="pb-[calc(16px+env(safe-area-inset-bottom))] sm:justify-end">
                  <Button variant="outline" className="rounded-full" onClick={() => setAssetModalOpen(false)} disabled={assetSaving}>Batal</Button>
            ...
            <Dialog open={assetDeleteOpen} onOpenChange={setAssetDeleteOpen}>
              <DialogContent className="bg-white w-[95vw] sm:max-w-md rounded-2xl p-0 overflow-hidden [&>button.absolute]:hidden">
                ...
                />
                <ModalContentWrapper className="space-y-2">
                  Hapus harta {assetDeleteTarget?.name || ''}?
                </ModalContentWrapper>
                <ModalFooter className="pb-[calc(16px+env(safe-area-inset-bottom))] sm:justify-end">
                  <Button variant="outline" className="rounded-full" onClick={() => setAssetDeleteOpen(false)} disabled={assetSaving}>Batal</Button>
            ...
            <Dialog open={assetDisposalOpen} onOpenChange={setAssetDisposalOpen}>
              <DialogContent className="bg-white w-[95vw] sm:max-w-lg rounded-2xl p-0 overflow-hidden [&>button.absolute]:hidden flex flex-col">
                  ...
                  {assetDisposalStatus !== 'AKTIF' ? (
                    <>
                        ...
                        {assetDisposalStatus === 'DIJUAL' ? (
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setAssetDisposalProceeds(parseRupiahToNumber(e.target.value).toString())}
                          ...
                          <div>
                            <Label>Harga Jual</Label>
                            <Input value="0" readOnly className="rounded-xl mt-1 bg-gray-100" />
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <Textarea value={assetDisposalNotes} onChange={(e) => setAssetDisposalNotes(e.target.value)} className="rounded-xl mt-1" />
                      </div>
                ...
                <ModalFooter className="flex-shrink-0 pb-[calc(16px+env(safe-area-inset-bottom))] sm:justify-end">
                  <Button variant="outline" className="rounded-full" onClick={() => setAssetDisposalOpen(false)} disabled={assetSaving}>Batal</Button>
            ...
            <Dialog open={assetDetailOpen} onOpenChange={setAssetDetailOpen}>
              <DialogContent className="bg-white w-[95vw] sm:max-w-3xl rounded-2xl p-0 overflow-hidden [&>button.absolute]:hidden flex flex-col">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                            ...
                            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                              ...
                              <div className="text-sm font-semibold text-gray-900 mt-1">
                                {disposedAt
                                  ? (String(assetDetailTarget.disposalType || 'SOLD') === 'SCRAPPED' ? 'Dihentikan' : 'Dijual')
                                  : 'Aktif'}
                              </div>
                              {disposedAt ? (
                              ...
                              <table className="min-w-[760px] w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                  ...
                                  {schedule.rows.length === 0 ? (
                                    <tr>
                                      <td colSpan={5} className="py-6 text-center text-sm text-gray-500">Tidak ada data</td>
                                    </tr>
                                  ) : null}
                                </tbody>
                              </table>
                ...
                <ModalFooter className="flex-shrink-0 pb-[calc(16px+env(safe-area-inset-bottom))] sm:justify-end">
                  <Button variant="outline" className="rounded-full" onClick={() => setAssetDetailOpen(false)}>Tutup</Button>
                </ModalFooter>
              </DialogContent>
            </Dialog>
          </TabsContent>
      ...
      <Dialog open={editBiayaOpen} onOpenChange={(v) => { setEditBiayaOpen(v); if (!v) setEditBiaya(null) }}>
        <DialogContent className="sm:max-w-[520px] p-0 overflow-hidden [&>button.absolute]:hidden">
          ...
          <ModalContentWrapper className="space-y-4">
            <div className="text-xs text-gray-500">
              {editBiaya?.source === 'MANUAL' ? 'Sumber: Biaya Manual' : 'Sumber: Kasir'}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3">
                {editBiaya?.source === 'MANUAL' ? (
                  ...
                  <div className="space-y-2">
                    <select
                      value={(biayaCategories.find(c => c.toLowerCase() === String(editKategori || '').trim().toLowerCase()) || '__CUSTOM__')}
                      onChange={(e) => {
                    ...
                    >
                      {biayaCategories.map((c) => (<option key={c} value={c}>{c}</option>))}
            ...
            <div className="text-[11px] text-gray-500">
              Perubahan kategori mempengaruhi “Biaya per Kategori” pada Laba Rugi.
            </div>
          </ModalContentWrapper>
          <ModalFooter className="sm:justify-between">
          ...
          <ModalContentWrapper className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                value={kategoriNew}
                onChange={(e) => setKategoriNew(e.target.value)}
              />
              <Button
                type="button"
      ...
      <Dialog open={taxSettingOpen} onOpenChange={setTaxSettingOpen}>
        <DialogContent className="sm:max-w-[680px] p-0 overflow-hidden [&>button.absolute]:hidden">
          ...
          <ModalContentWrapper className="space-y-4">
            ...
            <div className="text-xs text-gray-500">
              Skema Auto: omzet ≤ UMKM → Final, omzet ≤ Pasal 31E → 31E, selain itu → Standar.
            </div>
          </ModalContentWrapper>
          <ModalFooter className="sm:justify-between">
